...
public class FactoryImplementation<T>
	...
	CANSparkMax motor =
		new CANSparkMax(steerConfiguration.getMotorPort(), CANSparkMax.MotorType.kBrushless);
	...
	checkNeoError(
		motor.setPeriodicFramePeriod(CANSparkMax.PeriodicFrame.kStatus4, 500),
	...
	// Angle
	checkNeoError(
		motor.setPeriodicFramePeriod(CANSparkMax.PeriodicFrame.kStatus5, 500),
	...
	checkNeoError(
		motor.setPeriodicFramePeriod(CANSparkMax.PeriodicFrame.kStatus6, 500),
	...
	checkNeoError(
		integratedEncoder.setPositionConversionFactor(
			2.0 * Math.PI * mechConfiguration.getSteerReduction()),
		"Failed to set NEO encoder conversion factor");
	checkNeoError(
		integratedEncoder.setVelocityConversionFactor(
			2.0 * Math.PI * mechConfiguration.getSteerReduction() / 60.0),
		"Failed to set NEO encoder conversion factor");
	checkNeoError(
