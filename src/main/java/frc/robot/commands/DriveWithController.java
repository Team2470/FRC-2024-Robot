...
private final TrapezoidProfile.Constraints headingControllerConstraints =
	new TrapezoidProfile.Constraints(
		DriveConstants.kMaxAngularVelocityRadiansPerSecond / 4.0, 4 * Math.PI);
private final ProfiledPIDController headingController =
	new ProfiledPIDController(6.0, 0, 0, headingControllerConstraints);
...
public void execute() {
	...
	if (headingOverride != null) {
		...
		headingController.reset(
			new State(
				Math.toRadians(headingOverride), 0)); // TODO need to account for angular velocity
		}
		...
		SmartDashboard.putNumber(
			"DriveWithController - Heading error",
