...
//
	...
	private ArmFeedforward m_Feedforward =
		new ArmFeedforward(
			0, ShooterPivotConstants.kG, ShooterPivotConstants.kV, ShooterPivotConstants.kA);

	public ShooterPivot() {
		m_motor = new WPI_TalonFX(
			Constants.ShooterPivotConstants.MotorID, Constants.ShooterPivotConstants.MotorCANBus);
		m_motor.configFactoryDefault();
		m_motor.setInverted(true);
		m_motor.setNeutralMode(NeutralMode.Brake);

		m_encoder = new CANCoder(
			Constants.ShooterPivotConstants.EncoderID,
			Constants.ShooterPivotConstants.EncoderCANBus);
		...
		m_encoder.configSensorInitializationStrategy(
		...
		//
		// See
		// https://v5.docs.ctr-electronics.com/en/stable/ch18_CommonAPI.html?highlight=status%20frame#setting-status-frame-periods
		//
	...
	public void periodic() {
		...
		switch (m_controlMode) {
			case kOpenLoop:
				...
				outputVoltage = m_demand; break;

			case kPID:
				m_Feedforward = new ArmFeedforward(0,
				...
				// outputVoltage = kF * Math.cos(Math.toRadians(m_demand)) +
				// m_pidController.calculate(getAngle(), m_demand);
				double PIDoutPutVoltage =
					m_pidController.calculate(Math.toRadians(getAngle()), Math.toRadians(m_demand));
				...
				SmartDashboard.putNumber(
					"SP PID Profile Position", Math.toDegrees(m_pidController.getSetpoint().position));
				break;
		case kMusicMode:
			m_Orchestra.play(); break;
		default:
			// What happened!?
			break;
		}
		...
		SmartDashboard.putNumber(
			"Shooter Pivot" + " encoderAbosoluteAngle", m_encoder.getAbsolutePosition());
		SmartDashboard.putNumber("Shooter Pivot" + " encoderAngle", m_encoder.getPosition());
		SmartDashboard.putNumber(
			"Shooter Pivot" + " Motor Selected Sensor position", m_motor.getSelectedSensorPosition());
