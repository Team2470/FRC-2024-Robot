...
public class SimpleFlywheel extends SubsystemBase {
	...
	public SimpleFlywheel(int canID, boolean isLeft) {
		...
		//
		// See ths page for what each frame contains:
		// https://docs.revrobotics.com/sparkmax/operating-modes/control-interfaces#can-packet-structure
		//
		...
		// Default 200ms: Duty Cycle Absolute Encoder Position, Duty Cycle Absolute Encoder Absolute
		// Angle
	...
	public void periodic() {
		...
		SmartDashboard.putNumber(
			"Flywheel " + (m_isLeft ? "Left" : "Right") + " Velocity", getVelocity());
		SmartDashboard.putNumber(
			"Flywheel " + (m_isLeft ? "Left" : "Right") + " output voltage", outputVoltage);
		SmartDashboard.putNumber(
			"Flywheel " + (m_isLeft ? "Left" : "Right") + " RPM Error", getErrorRPM());
		SmartDashboard.putNumber(
			"Flywheel " + (m_isLeft ? "Left" : "Right") + " RPM Percent Error", getErrorPercent());
		SmartDashboard.putString(
			"Flywheel " + (m_isLeft ? "Left" : "Right") + " Control Mode", m_controlMode.toString());
	...
	public Command feederShooterCommand(SimpleShooterFeeder m_feeder) {
		return Commands.repeatingSequence(
			this.waitUntilErrorInrange(), m_feeder.forward().until(() -> this.isErrorOutOfRange()));
	}
	/**
	...
	*/
	...
	public Command pidCommand(DoubleSupplier rpmSupplier) {
		return Commands.runEnd(() -> this.setPIDSetpoint(rpmSupplier.getAsDouble()), this::stop, this);
	}
